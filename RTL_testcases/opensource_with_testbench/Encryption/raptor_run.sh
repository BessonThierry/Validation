...
#creating a tcl file to run raptor flow 
    ...
    [ -z "$ip_name" ] && echo "add_design_file ./ rtl/AddRoundKey.v">>raptor_tcl.tcl || echo "" 
    [ -z "$ip_name" ] && echo "add_design_file ./ rtl/KeyExpansion.v">>raptor_tcl.tcl || echo "" 
    [ -z "$ip_name" ] && echo "add_design_file ./ rtl/MixColumns.v">>raptor_tcl.tcl || echo "" 
    [ -z "$ip_name" ] && echo "add_design_file ./ rtl/ShiftRows.v">>raptor_tcl.tcl || echo "" 
    [ -z "$ip_name" ] && echo "add_design_file ./ rtl/SubBytes.v">>raptor_tcl.tcl || echo "" 
    ...
    if [ "$post_synth_sim" == true ] || [ "$post_route_sim" == true ] || [ "$bitstream_sim" == true ]; then
        echo "add_simulation_file ./sim/co_sim_tb/co_sim_$design.v rtl/$design.v rtl/AddRoundKey.v rtl/KeyExpansion.v rtl/MixColumns.v rtl/ShiftRows.v rtl/SubBytes.v">>raptor_tcl.tcl 
